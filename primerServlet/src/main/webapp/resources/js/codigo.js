...
function listarUsuarios() {
		...
		function enviarDatos() {
			...
			fetch(`endpoint?nombreInput=${nombre}&apellidoInput=${apellido}&emailInput=${email}&fechaInput=${fecha}&temaInput=${tema}`, {
				method: 'POST'
		...
		function renderizarCard(data) {
			...
			if (cards.length >= 3) {
				cardContainer.removeChild(cards[0]); // Elimina la card más antigua si hay tres o más
			}
